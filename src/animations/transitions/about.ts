...
const setup = ({
  ...
  tlInterests: gsap.core.Timeline;
  contentInterests: HTMLDivElement;
...
const setupSectionsAnimation = ({
  ...
  sectionsMm = createMatchMedia((_context, { isLandscape }) => {
    ...
    if (isLandscape) {
      ...
      // Interests animation
      tl.fromTo(contentInterests, { opacity: 0 }, { opacity: 1, duration: 0.12, ease: "power1.out" }, INTERESTS_DELAY);
      tl.add(() => {
        tlInterests?.play();
      }, INTERESTS_DELAY);
    } else {
      ...
      tl.to(contentServices, { opacity: 0, y: "-10vh", duration: 0.15, ease: "power1.out" }, INTERESTS_DELAY - 0.075);
      tl.add(() => {
        tlServices?.play();
      }, SERVICES_DELAY);
